...
const VideoGenerator = forwardRef<VideoGeneratorHandle, VideoGeneratorProps>(
  ({ title, token, enrichedTranscriptions, audioBlob }, ref) => {
    ...
    const [phase, setPhase] = useState<
      'idle' | 'generating' | 'composing' | 'done'
    >('idle');
    const [composedVideoUrl, setComposedVideoUrl] = useState<string | null>(
      null,
    );
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-background md:p-6 lg:p-8">
        ...
        {/* Card */}
        <div className="scanlines relative mt-16 mb-20 z-10 w-full max-w-5xl rounded-2xl border border-(--border-hi) bg-surface shadow-[0_24px_64px_rgba(0,0,0,0.6)] md:mb-0">
          {/* Header */}
          <div className="flex flex-col items-center justify-between gap-4 p-5 border-b border-border md:flex-row md:px-7">
            <div className="flex items-center gap-3.5">
              ...
              <Logo />
              <div>
                <h1 className="font-['Syne'] text-[17px] font-bold text-(--text-1) leading-tight tracking-tight">
                  Video Composer
                </h1>
                <p className="text-[11px] text-(--text-3) tracking-wide mt-px">
                  {total} scene{total !== 1 ? 's' : ''} ·{' '}
                  {audioBlob ? 'Audio attached' : 'No audio'}
                </p>
              </div>
            </div>

            {/* Status badge */}
            <div className="flex items-center gap-2 pl-2.5 pr-1.5 py-1.5 rounded-full border border-(--border-hi) bg-elevated">
              <span
                className={[
              ...
              />
              <span className="font-mono text-[10px] uppercase tracking-wider text-(--text-2) px-1">
                {phase}
              </span>
            </div>
          </div>
              ...
              <div className="flex flex-col gap-8 animate-fade-up">
                ...
                <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4">
                  ...
                  <button
                    ...
                    className="w-full sm:w-auto px-8 py-3.5 rounded-full border border-(--border-hi) bg-elevated text-(--text-1) font-['Syne'] font-bold text-sm hover:bg-overlay transition-all flex items-center justify-center gap-2"
                  >
                    <svg
                      width="18"
                      height="18"
            ...
            {phase === 'done' && !composedVideoUrl && !isRendering && (
              <div className="flex flex-col gap-8 animate-fade-up">
                <div className="p-6 rounded-2xl bg-error bg-opacity-5 border border-error border-opacity-20 flex items-center gap-4">
                  <div className="shrink-0 w-10 h-10 rounded-full bg-error flex items-center justify-center text-white">
                    <svg
                      width="20"
                      height="20"
                      viewBox="0 0 24 24"
                  ...
                  <div>
                    <h3 className="text-(--text-1) font-bold font-['Syne']">
                      Composition Failed
                    </h3>
                    <p className="text-(--text-3) text-xs font-mono">
                      Individual scenes were generated successfully, but
                      stitching them together failed. You can still view the
                      scenes below.
                    </p>
                  </div>
                </div>
